    ...
    methods: {
      ...
      async fetchChanges() {
        const results = await Promise.all([
          requestWithToken(`${monitorServerUrl}/report?timestamp=${G.lastUpdatedTimestamp}`),
        ...
        for (let i = 0; i < activeNodesIds.length; i++) {
          data.nodes.active[activeNodesIds[i]].nodeIsFoundationNode = listOfFoundationNodes.includes(
      ...
      updateNetworkStatus(report) {
        ...
        for (let nodeId in report.nodes.active) {
          ...
          this.nodeLoads.push({
            ...
            loadInternal: node.currentLoad?.nodeLoad?.internal || 0,
            loadExternal: node.currentLoad?.nodeLoad?.external || 0,
      ...
      async deleteRemovedNodes() {
        ...
        try {
          ...
          G.visEdges.remove(removedNodeIds.map((id) => this.getVisEdgeId(`eoa-${id}`, id)))
        } catch (e) {
      ...
      async updateNodes() {
        try {
          ...
          console.log(
            `Total of ${Object.keys(changes.nodes.active).length}/${Object.keys(G.nodes.active).length} nodes updated.`
          )
      ...
      async getActiveArchivers() {
        ...
        const res = await requestWithToken(`http://${G.archiver.ip}:${G.archiver.port}/archiverlist`)
      ...
      visContextRenderer({ ctx, id, x, y, style }) {
        ...
        const indicator = currentNode != null ? currentNode.lastScalingTypeRequested : undefined
      ...
      isUnexpectedOOS(node, CAndCEOnly = false) {
        ...
        if (node.radixes && typeof node.radixes[Symbol.iterator] === 'function') {
          for (let radix of node.radixes) {
            ...
            if (!radix.insync) {
              const recentlyActive = currentCounter - node.cycleFinishedSyncing <= this.isRecentActiveCycles
      ...
      async start() {
        ...
        for (let i = 0; i < activeNodesIds.length; i++) {
          ...
          report.nodes.active[nodeId].nodeIsFoundationNode = listOfFoundationNodes.includes(
        ...
        G.network.on('click', (params) => {
          ...
          let node =
            G.nodes.active[nodeId] || G.nodes.syncing[nodeId] || G.nodes.joining[nodeId] || G.nodes.standby[nodeId]
          if (!node) return
