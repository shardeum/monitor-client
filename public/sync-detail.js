    ...
    methods: {
      ...
      getBackgroundColor(r) {
        ...
        if (r.recentRuntimeSync) {
          ...
          return r.insync ? this.syncColors.insync[colorKey] : this.syncColors.outofsync[colorKey]
        }
      },
      ...
      generateFullRadixArray(node) {
        const validRadixes = node.radixes.map((r) => parseInt(r.radix, 16)).filter((r) => !isNaN(r) && r < 1e6)
      ...
      async start() {
        ...
        for (let i = 0; i < activeNodesIds.length; i++) {
          report.nodes.active[activeNodesIds[i]].nodeIsFoundationNode = listOfFoundationNodes.includes(
