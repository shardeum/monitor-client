...
const NetworkMonitor = function (config) {
    ...
    for (const publicKey in report.nodes.joining) {
      ...
      if (!G.joining[publicKey]) {
        ...
        if (joiningCount <= maxJoiningNodes) {
          ...
          G.joining[publicKey].tooltipInstance = drawTooltipForInactiveNodes(G.joining[publicKey])
        } else {
    ...
    for (const nodeId in report.nodes.syncing) {
      ...
      if (!G.syncing[nodeId] && nodeId !== null && nodeId !== 'null') {
        if (G.joining[publicKey]) {
          ...
          G.syncing[nodeId].tooltipInstance = drawTooltipForInactiveNodes(G.syncing[nodeId])
          if (G.standby[nodeId]) {
          ...
          G.syncing[nodeId] = createNewNode('syncing', nodeId, [], report.nodes.syncing[nodeId].nodeIpInfo)
    ...
    for (const nodeId in report.nodes.active) {
      ...
      if (
        ...
        if (G.syncing[nodeId]) {
          ...
          try {
            G.active[nodeId].appState = report.nodes.active[nodeId].appState || '00ff00ff'
          ...
          try {
            ...
            G.active[nodeId].externalPort = report.nodes.active[nodeId].nodeIpInfo.externalPort
            G.active[nodeId].shardusVersion = report.nodes.active[nodeId].shardusVersion
            G.active[nodeId].lastScalingTypeWinner = report.nodes.active[nodeId].lastScalingTypeWinner
            G.active[nodeId].lastScalingTypeRequested = report.nodes.active[nodeId].lastScalingTypeRequested
  ...
  const injectTransactions = function () {
    ...
    for (const nodeId in G.active) {
      ...
      const injectInterval = setInterval(() => {
        ...
        transformCircle(injectedTx.circle, node.currentPosition.x, node.currentPosition.y, null, G.txAnimationSpeed)
  ...
  const updateTables = function () {
    ...
    if (Object.keys(G.active).length > 0) {
      ...
      const currentCycleCounter = Math.round(G.active[Object.keys(G.active)[0]].cycleCounter || 0)
  ...
  const drawTooltip = function (node) {
    ...
    node.circle.on('mouseover', () => {
      ...
      const cycleMarkerShort = `${node.cycleMarker.slice(0, 4)}...${node.cycleMarker.slice(59, 63)}`
      const appStateShort = `${node.appState.slice(0, 4)}...${node.appState.slice(59, 63)}`
      const nodeListShort = `${node.nodelistHash.slice(0, 4)}...${node.nodelistHash.slice(59, 63)}`
